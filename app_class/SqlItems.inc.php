<?php
/*  --------------------------------------------------------------------------------------------
		Nombre              : SqlItems.inc.php
		Autor               : Jose Carlos Garcia 
		Mejorado            : Ing. Oscar Amesty V.
		Fecha Creación      : 25/02/2008
		Objetivo / Resumen  : Funciones que generan sentencias SQL bien formadas
...
function get_select($table, $columns, $where='', $order='',$distinct=false){
	$tmp = "SELECT ";
	if($distinct) $tmp .= "DISTINCT ";
	$tmp .= "$columns FROM $table";
	if($where!='') $tmp .= " WHERE $where";
	if($order!='') $tmp .= " ORDER BY $order";
	return $tmp;
}
