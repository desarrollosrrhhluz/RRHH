...
$pdf->SetFont('helvetica', '', 12);
 ...
 $cant=1;
 $sqlP="select *,EXTRACT(year FROM age(timestamp 'now()',date(fe_nacimiento) ) ) as edad from admon_personal.tab_resumen_funeraria where parentesco!='TITULAR' and ce_relacion=$cedula and anno+mes=(select max(anno+mes) from admon_personal.tab_resumen_funeraria) order by parentesco ASC";
    ...
    $ds     = new DataStore($db, $sqlP);
     ...
     $i=0;
        ...
        while($i<$j){
          ...
          <td width="15%" align="center">'.$row['ce_beneficiario'].'</td> 
          <td width="20%" align="center" >'.$row['parentesco'].'</td> 
...
entendiéndose como parte integrante del grupo de beneficiarios: el titular, los padres, cónyuge o persona con la que mantiene unión estable de hecho, hijos, sin límite de edad,  
...
personas del GRUPO FAMILIAR, con atención las 24 horas desde el día 01/01/2018 al 31/12/2018 .<br>
<br>

* Los datos suministrados no podr&aacute;n ser modificados hasta el vencimiento del contrato (31/12/2018), salvo ocurra el nacimiento de un hijo realizando la participaci&oacute;n ante la Direcci&oacute;n de Recursos Humanos.<br> 
