...
class PDF extends FPDF {
 ...
 $sql    = "SELECT *, convert(char(12), FE_INGRESO, 103) as fecha FROM VW_CABECERA_ACTUAL 
        WHERE CE_TRABAJADOR = $this->cedula AND SUBSTRING(DEDICACION,1,1) > '0'";
        ...
        if($fila["FE_JUBILACION"] <> NULL){
            $this->SetFont('Arial','B',14);
            $this->Ln(40);
            $this->Cell(180,10,'Jubilados no pueden realizar consultas para Estimado de Prestaciones Sociales',0,0,'C');
        }
        else {
//            $fila = $ds0->getValues(0);
